        ...
        match panel {
            ...
            Panel::DAGRun => {
                if let Some(dag_id) = self.nav_context.dag_id() {
                    ...
                    // Backfill the dags panel cache so triggering from the DAG list also has params
                    if let Some(ref params) = self.dagruns.dag_params {
                        self.dags
                            .dag_params_cache
                            .insert(dag_id.clone(), params.clone());
                    }
